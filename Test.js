...
class Test { //Класс - Тест
    constructor(number1,number2,answer1,answer2) {
        ...
        while((this.answer1 == this.answer2) || (this.answer1 == this.correct_answer))
    }

    CreateTest(option){
        if (option == "+"){
            question.innerHTML = this.number1 + "+" + this.number2 + "="; // выводим вопрос
            this.correct_answer = this.number1+this.number2;              // считаем сумму
        }
        if(option == "-"){
            ...
            this.correct_answer = this.number1-this.number2;              // считаем вычитание
            question.innerHTML = this.number1 + "-" + this.number2 + "="; // выводим вопрос
        }
        if(option == "*"){
            question.innerHTML = this.number1 + "∙" + this.number2 + "="; // выводим вопрос
            this.correct_answer = this.number1*this.number2;              // считаем умножение
        }
        if(option == ":"){
            ...
            this.correct_answer = this.number1/this.number2;              // считаем деление
            question.innerHTML = this.number1 + ":" + this.number2 + "="; // выводим вопрос
        }
